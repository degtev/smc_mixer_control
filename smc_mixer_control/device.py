...
import time
from smc_mixer_control.controls import *
from smc_mixer_control.device_channel import DeviceChannel
...
class Device(object):
    ...
    def __init__(self, dev_info):
        ...
        with open(filename, 'r', encoding='utf-8') as device_file:
        ...
        self.unique_id = self.config.get("unique_id", self.config.get("name", "unknown"))
    ...
    def build_control(self, conf):
        if conf["type"] == "fader":
            ...
            return Fader(
                ...
                feedback=conf["fb"],
                midi_value_min=conf.get("min", 0),
                midi_value_max=conf.get("max", 127)
            )
        elif conf["type"] == "knob":
            mtype = self.inflate(conf["mtype"])
            m_id_field,m_val_field = self.look_up_fields(mtype)
